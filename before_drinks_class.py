...
def add_faves_class():
    ...
    for i,s in zip(load_into_list(fave_dictionary,'name'),load_into_list(fave_dictionary,'favourites')):
        F1 =classes_previous.Favourites(i)
...
def load_people():
    with open(file_people_list,'r') as f:
        my_file = f.readlines()
        for i in my_file:
            people.append(classes_previous.Person(i.strip()))
...
def add_name():
    ...
    if user_input_name not in print_people():
        ...
        people.append(classes_previous.Person(user_input_name))
    else:
